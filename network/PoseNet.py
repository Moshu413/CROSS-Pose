...
from network.backbone import convnext_backbone  # 引入主干网络
...
from .pose_utils.rot_reps import rot6d_to_mat_batch, euler2mat_batch, rot6d_fixed_y_to_mat_batch, \
    rot6d_fixed_z_to_mat_batch, rot6d_fixed_x_to_mat_batch
...
class PoseNet(nn.Module):
    def __init__(self):
        ...
        self.pnp_net = ConvPnPNet(pnp_input_feat, featdim=featdim, mask_attention_type=FLAGS.mask_attention_type,
                                  rot_dim=rot_dim, flat_op=FLAGS.flat_op)
        ...
        self.synset_names = ['bottle', 'bowl', 'camera', 'can', 'laptop', 'mug']  # 根据你的类别修改
        self.description = [
            "The bottle typically has a cylindrical shape with a narrow neck, a wider body, and a flat or slightly rounded base.",
            'The bowl has a circular rim, a rounded concave interior, and a gently curved or flat base.',
        ...
        local_clip_path = "/media/ailab/lsy/GIVEPose-master/pretrained/clip-vit-base-patch16"
        self.clip_processor = CLIPProcessor.from_pretrained(local_clip_path)
        self.clip_model = CLIPModel.from_pretrained(local_clip_path)
        ...
        self.feature_fusion = nn.Sequential(
            nn.Conv2d(feature_channel + 256, feature_channel, 1),  # 假设要融合到主干特征
            nn.ReLU(),
        )

    def extract_text_features(self, category_labels):

        batch_size = category_labels.shape[0]

        texts = []
        for i in range(batch_size):
            cls_idx = category_labels[i].item()
            text = f'A photo of a {self.synset_names[cls_idx]}. {self.description[cls_idx]}'
            texts.append(text)

        with torch.no_grad():
            inputs = self.clip_processor(text=texts, return_tensors="pt", padding=True)
            inputs = {key: value.to(category_labels.device) for key, value in inputs.items()}
            text_features = self.clip_model.get_text_features(**inputs)  # [B, 512]

        text_features = self.clip_mlp(text_features)  # [B, 256]
    ...
    def forward(self, data, device, do_loss=False, pred_scale=None):
        ...
        if 'category_label' in data:
            category_labels = data['category_label'].to(device)
        else:
            category_labels = torch.zeros(img.size(0), dtype=torch.long).to(device)
        ...
        text_features = self.extract_text_features(category_labels)
        ...
        if self.backbone_name == 'convnext':
            ...
            text_features_broadcast = text_features.expand(b, 256, h, w)

            fused_feat = torch.cat([feat[0], text_features_broadcast], dim=1)
            fused_feat = self.feature_fusion(fused_feat)

            feat = (fused_feat,) + feat[1:] if len(feat) > 1 else (fused_feat,)
        ...
        text_emb = text_features.squeeze(-1).squeeze(-1)  # [B, 256]
        size_offset = self.size_offset_head(text_emb)  # [B, 3]
        pred_size = pred_size + norm_mean_size + 0.1 * size_offset
